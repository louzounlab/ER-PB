...
def home_page():
    if request.method == 'POST':
        ...
        try:
            ...
            if ht_measure == 'foot':
                features_dictonary['ht'] = float(str(request.form['ht']).split('\'')[0]) * 30.48 + float(
                    str(request.form['ht']).split('\'')[1]) * 2.54
            else:
        ...
        if error:
            ...
            for sample in close_samples:
                risks.append(
                    np.mean(np.array(tag.iloc[:, int(model) - 1][
                                     int(sample - 0.02 * df.shape[0]):int(sample + 0.02 * df.shape[0])])))
